<?php
/**
 * WP Stocks Manager — 管理画面: 設定ページ（スケジュール／手動実行／メンテナンス／外部API連携／ニュースRSS／複合シグナル判定の各タブ）
...
function wp_stocks_settings_page() {
    ...
    if (isset($_POST['wp_stocks_save_settings'])) {
		...
		// J-Quants APIキー保存
        $jquants_api_key = sanitize_text_field($_POST['jquants_api_key'] ?? '');
        update_option('wp_stocks_jquants_api_key', $jquants_api_key);
    ...
    echo '<tr><th>手動 J-Quants同期（33業種・市場区分・予想EPS等）</th><td>';
    echo '<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">';
    ...
    echo '</div>';
    echo '<p class="description" style="margin-top:8px;">通常は日曜22:00の企業情報週次Cronの一部として自動実行されます。設定変更後の動作確認など、次回Cronを待たずに反映したい場合にご利用ください。銘柄間に1秒のsleepを挟みます。</p>';
    echo '</td></tr>';
	...
	// J-Quants APIキー設定
    $jquants_api_key = get_option('wp_stocks_jquants_api_key', '');
	...
	$jpx_latest = $wpdb->get_var("SELECT MAX(report_month) FROM {$wpdb->prefix}stock_jpx_sector_per");
